...
int create_expression_tree(BiTree * tree, BiTreeNode * parent, BiTreeNodeSide side, char *p, int l){    //构造表达式树
    ...
    if(rpst1 < 0) {     //表达式中不存在* / % ^运算符
        if(*p == '(') { //表达式头为左括号，此时表达式整体被括号括起来
            ...
            else if(rpst3 > 0){     //此时表达式首为操作符sin cos等，其余部分被一对括号括起来
                char *value = (char *)malloc(4*sizeof(char));
                strncpy(value, p, 3);
                *(value + 3) = '\0';
                BiTreeNode * newNode = bitree_insert(tree, value, parent, side);
	...
	else {	//表明表达式是几个因子 * / % ^ 而组成的
        /*if(*(p+rpst3+1) == '('){    //有* / % ^号，但是位于sin的括号内
            char *value = (char *)malloc(4*sizeof(char));
            strncpy(value, p+rpst3-2, 3);
            *(value + 3) = '\0';
            BiTreeNode * newNode = bitree_insert(tree, value, parent, side);
        ...
        *(value+1) = '\0';
        ...
        if(*(p+rpst1+1) == 's' || *(p+rpst1+1) == 'c' || *(p+rpst1+1) == 't' || *(p+rpst1+1) == 'e'){ //  * / % ^后面跟着sin tan等运算符
            char *value = (char *)malloc(4*sizeof(char));
            strncpy(value, p+rpst1, 1);
            *(value+1) = '\0';
            BiTreeNode *newNode = bitree_insert(tree, value, parent, side);
